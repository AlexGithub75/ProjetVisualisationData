export default function renderProtocolPieChart(svg, data) {
    // Dimensions de l'SVG avec valeurs par défaut
    const width = +svg.attr('width') || 600;
    const height = +svg.attr('height') || 600;
    const radius = Math.min(width, height) / 2 - 40;

    // Agrégation des données par Protocol
    const protocolCounts = d3.rollup(data, v => v.length, d => d['Protocol']);
    const pieData = Array.from(protocolCounts, ([key, value]) => ({ key, value }));
    ...
    const color = d3.scaleOrdinal()
        ...
        .range(['#FFA500', '#1E90FF', '#32CD32']); // Orange pour UDP, Bleu pour ICMP, Vert pour TCP
